...
[CreateAssetMenu(menuName = "Parts/Component")]
public class Component : ScriptableObject
{
    public string Name = "";

    [SerializeField]
    public ComponentDictionary RequiredComponents = new ComponentDictionary();

    public int GetNumberNeeded(Component t)
    {
        var thing = RequiredComponents.FirstOrDefault(x => x.Key.Name == t.Name);
        try
        {
            return thing.Value;
        }catch
    ...
    public void AddRequirementFor(Component c, int amount)
    {
        RequiredComponents.Add(c, amount);
    }


    public void AddNewRequirement(Component c = null, int count = 0)
    {
        int existingAmount = GetNumberNeeded(c);
        if(existingAmount == 0)
        {
            AddRequirementFor(c, count);
        }
    }
...
[Serializable]
public class ComponentDictionary : SerializableDictionary<Component, int> { }

[CustomPropertyDrawer(typeof(ComponentDictionary))]
public class ComponentDictionaryDrawer : DictionaryDrawer<Component, int> { }